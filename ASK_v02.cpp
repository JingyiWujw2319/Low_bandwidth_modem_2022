    ...
    if (waveform_mode==0){
        ...
        if(sendingPacket){
            ...
            if(sine_index>=WAVEBUFFERLENGTH-1){
                sine_index = 0;
                // This means a bit has been transmitted 
                if(bitIndex>7){
                    // This means a byte has been transmitted 
                    if(!messageBuffer->empty()){
                        currentByte = messageBuffer->front();
                        bitIndex = 0; 
                        currentBit = bitRead(currentByte,bitIndex);
                        messageBuffer->pop();
                    }else{
                        currentBit = 0; 
                        sendingPacket = false;
                    ...
                    // Still transmitting current byte 
                    currentBit = bitRead(currentByte,bitIndex);
                    bitIndex ++;
                    Serial2.println(currentBit);
                }
            }else{
                if(currentBit == 1){
